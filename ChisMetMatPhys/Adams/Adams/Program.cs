...
namespace lab1RK
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            for (int i = 4; i <= n; i++)
            {
                yStar[i] = defaultY[i - 1] + h / 24.0 * (55 * Runge_Kutt.Function(Xs[i - 1], defaultY[i - 1]) - 59 * Runge_Kutt.Function(Xs[i - 2], defaultY[i - 2]) + 37 * Runge_Kutt.Function(Xs[i - 3], defaultY[i - 3]) - 9 * Runge_Kutt.Function(Xs[i - 4], defaultY[i - 4]));
            }
            for (int i = 0; i < yStar.Count; i++)
            {
                //Console.WriteLine("y*[{0}]={1}", i, yStar[i]);
            }
            ...
            for (int i = 3; i <= n; i++)
            {
                Adams_Moulton[i] = Adams_Moulton[i - 1] + h / 24.0 * (9 * Runge_Kutt.Function(Xs[i], yStar[i]) + 19 * Runge_Kutt.Function(Xs[i - 1], Adams_Moulton[i - 1]) - 5 * Runge_Kutt.Function(Xs[i - 2], Adams_Moulton[i - 2]) + Runge_Kutt.Function(Xs[i - 3], Adams_Moulton[i - 3]));
            }
